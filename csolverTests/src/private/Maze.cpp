...
int MazeSolver::solve(int times, int numRows, int numCols, int seed, bool printVerbose)
{
	...
	CS_LOG("TestMaze(%d)", solver.getSeed());
...
int MazeSolver::check(const shared_ptr<TTopologyVertexData<VarID>>& tileData, const ConstraintSolver& solver)
{
	...
	reachable.resize(grid->getNumNodes(), false);
	...
	for (int y = 0; y < numRows; ++y)
	{
		if (PRINT_EDGES && y != 0)
		{
			...
			for (int x = 0; x < numCols; ++x)
			{
				...
				int edgeNode = edgeTopology->getNodeForSourceEdge(node, upNode);
		...
		for (int x = 0; x < numCols; ++x)
		{
			...
			if (PRINT_EDGES && x != 0)
			{
				...
				int edgeNode = edgeTopology->getNodeForSourceEdge(node, leftNode);
				vector<int> edgeVals = solver.getPotentialValues(edges->get(edgeNode));
				if (edgeVals.empty())
				{
					out += TEXT("!");
