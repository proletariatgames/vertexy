...
int TowersOfHanoiSolver::solveTowersGrid(int times, int N, int seed, bool bPrint)
{
	int nErrorCount = 0;
	for (int time = 0; time < times; ++time)
	{
		...
		for (int turn = 0; turn < turns; ++turn)
		{
			if (turn == 0 || turn == turns-1)
			{
				...
				for (int i = 0; i < N; ++i)
				{
					fullPeg.push_back(solver.makeVariable({ wstring::CtorSprintf(), TEXT("%d-%d-%d"), fullPegInd, turn, i }, { N - i }));
				}
		...
		for (int peg = 0; peg < numPegs; ++peg)
		{
			for (auto peg : pegs[peg])
			{
				for (int i = 1; i < N; ++i)
				{
					solver.inequality(peg[i], EConstraintOperator::LessThanEq, peg[i - 1]);
				}
			}
		}

		hash_map<int, tuple<int, int>> cardinalities;
		cardinalities[0] = make_tuple(2 * N, 2 * N);
		for (int i = 1; i <= N; ++i)
		{
		...
		for (int turn = 1; turn < turns; ++turn)
		{
			for (int peg = 0; peg < numPegs; ++peg)
			{
				...
				for (int i = 0; i < N; ++i)
				{
					...
					for (int ii = 1; ii <= N; ++ii) //doesn't apply to 0
					{
						for (int jj = 0; jj <= N; ++jj)
						{
							if (ii != jj)
		...
		if (bPrint) print(N, &solver, pegs, moved);
		nErrorCount += check(N, &solver, pegs);
...
void TowersOfHanoiSolver::print(int N, ConstraintSolver* solver, const vector<vector<VarID>>* vars, const vector<VarID>& moved)
{
	const int turns = (1 << N); //2^n
	const int numPegs = 3;
	...
	for (int turn = 0; turn < turns; ++turn)
	{
		...
		{
			CS_LOG("-----%d", solver->getSolvedValue(moved[turn - 1]));
		}
...
void TowersOfHanoiSolver::printDiskBased(int numDisks, ConstraintSolver* solver, const vector<VarID>& move, const vector<VarID>& moveDest, const vector<vector<VarID>>& diskOn)
{
	...
	for (int turn = 0; turn < numTurns; ++turn)
	{
		...
		CS_LOG("Move %d->%d", solver->getSolvedValue(move[turn]), solver->getSolvedValue(moveDest[turn]));
	}
}

void moveBetween(int N, vector<vector<int>>* pegs, int fromTurn, int xPeg, int yPeg)
{
	int topX = N;
	int topY = N;
	for (int i = N - 1; i >= 0; --i)
...
int TowersOfHanoiSolver::check(int N, ConstraintSolver* solver, const vector<vector<VarID>>* vars)
{
	int nErrorCount = 0;
	...
	const int pegA = 0;
	const int pegB = N % 2 ? 1 : 2;
	const int pegC = N % 2 ? 2 : 1;

	vector<int> fullPeg;
	fullPeg.reserve(N);
	for (int i = 0; i < N; ++i)
	{
		fullPeg.push_back(N - i);
	}
	...
	for (int turn = 1; turn < turns; ++turn)
	{
		...
		if (turnType == 0)
		{
			moveBetween(N, correctSequence, turn - 1, pegA, pegB);
		}
		else if (turnType == 1)
		{
			moveBetween(N, correctSequence, turn - 1, pegA, pegC);
		}
		else
		{
			moveBetween(N, correctSequence, turn - 1, pegB, pegC);
		}
