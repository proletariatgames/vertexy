...
SolverDecisionLevel ConflictAnalyzer::analyzeConflict(SolverTimestamp conflictTs, ISolverConstraint* conflictingConstraint, VarID contradictingVariable, ClauseConstraint*& outLearned)
{
	...
	cs_assert(m_solver.hasFinishedInitialArcConsistency());
	...
	{
		//CS_LOG("** Contradiction detected (no variable specified)");
		...
		else
		{
			...
			{
				CS_WARN("Constraint could not explain failure!");
	...
	{
		// CS_LOG("** Contradiction on %s", Solver.VariableDB.GetVariableName(ContradictingVariable).c_str());
		cs_sanity(m_solver.m_variableDB.isInContradiction(contradictingVariable));
	...
	// CS_LOG("Initial conflict explanation: %s", Solver.LiteralArrayToString(Explanation).c_str());
	...
	cs_sanity(!containsPredicate(explanation.begin(), explanation.end(), [&](auto& lit) { return !lit.values.contains(false); }));
...
SolverDecisionLevel ConflictAnalyzer::searchImplicationGraph(vector<Literal>& inOutExplanation, const ISolverConstraint* initialConflict, SolverTimestamp conflictTime)
{
	...
	cs_assert(m_solver.getCurrentDecisionLevel() == decisionLevelAtConflict);
	...
	m_anchorGraphNode = initialConflict->getGraphRelationInfo() != nullptr ? initialConflict->getGraphRelationInfo()->sourceGraphNode : -1;
	...
	cs_assert(!m_nodes.empty());
	for (int i = 0; i < m_nodes.size(); ++i)
	{
		cs_assert(m_nodes[i].var == inOutExplanation[i].variable);
	...
	cs_sanity(assignmentStack[mostRecentDecisionAssignment].constraint == nullptr);
	...
	while (!m_nodes.empty() && (m_numTopLevelNodes > 1 || m_nodes[mostRecentNodeIndex].time > mostRecentDecisionAssignment))
	{
		...
		cs_assert(m_nodes[mostRecentNodeIndex].var == inOutExplanation[mostRecentNodeIndex].variable);
		cs_assert(lastModificationTime == findLatestFalseTime(m_nodes[mostRecentNodeIndex].var, inOutExplanation[mostRecentNodeIndex].values, lastModificationTime));
		...
		cs_assert(mod.variable == pivotVar);
		...
		//CS_LOG("Antecedent constraint explanation on pivot %s at %d: %s", *db.GetVariableName(PivotVar).ToString(), LastModificationTime, *Solver.LiteralArrayToString(ExplToResolve));
		...
		//CS_LOG("Resolved to %s", *Solver.LiteralArrayToString(InOutExplanation));
	...
	cs_assert(m_nodes.size() == inOutExplanation.size());
	cs_assert(m_nodes[mostRecentNodeIndex].var == inOutExplanation[mostRecentNodeIndex].variable);
	cs_sanity(m_nodes[mostRecentNodeIndex].time == findLatestFalseTime(m_nodes[mostRecentNodeIndex].var, inOutExplanation[mostRecentNodeIndex].values, m_nodes[mostRecentNodeIndex].time));
	...
	else
	{
		...
		if (m_graph != nullptr)
		{
			cs_sanity(initialConflict->getGraphRelationInfo() && initialConflict->getGraphRelationInfo()->graph == m_graph);
			...
			if (isPromotable)
			{
				m_resolvedRelationInfo = move(make_unique<ConstraintGraphRelationInfo>(m_graph, m_anchorGraphNode));
				for (int i = 0; i < m_nodes.size(); ++i)
				{
					...
					if (!isClauseRelation(node.relation))
					{
						...
						VarID resolvedVar;
						cs_sanity(resolvedRel->getRelation(m_anchorGraphNode, resolvedVar) != false);
						cs_sanity(resolvedVar == node.var);
					}
					else
					{
						auto resolvedRel = get<LiteralRelationType>(node.relation);
						Literal resolvedLit;
						cs_sanity(resolvedRel->getRelation(m_anchorGraphNode, resolvedLit) != false);
						cs_sanity(resolvedLit == inOutExplanation[i]);
					}
					#endif
				}
			}
		...
		if constexpr (REDUNDANCY_CHECKING_LEVEL > 0)
		{
			...
			{
				// CS_LOG("Reduced clause size from %d to %d", InOutExplanation.Num(), NewSize);
				// CS_LOG("Old: %s", *Solver.LiteralArrayToString(InOutExplanation));

				inOutExplanation.resize(newSize);
				m_nodes.resize(newSize);

				// CS_LOG("New: %s", *Solver.LiteralArrayToString(InOutExplanation));
			}
		}
		...
		else
		{
			...
			cs_assert(m_nodes[1].var == inOutExplanation[1].variable);
			...
			{
				cs_sanity(inOutExplanation[i].variable == m_nodes[i].var);
			...
			cs_assert(bestLevel < m_solver.getDecisionLevelForTimestamp(uipTime));
			return bestLevel;
		}
	}
}
...
int ConflictAnalyzer::findMostRecentNodeIndex() const
{
	...
	for (int i = 1; i < m_nodes.size(); ++i)
	{
		cs_sanity(m_solver.getDecisionLevelForTimestamp(m_nodes[i].time) <= m_topLevel);
...
void ConflictAnalyzer::resolve(const vector<Literal>& newClauses, const ConstraintGraphRelationInfo& relationInfo, vector<Literal>& outClauses, VarID pivotVar, SolverTimestamp newClauseTimestamp)
{
	...
	{
		cs_assert(newClausePivotIndex >= 0);

		cs_assert(outClauses[pivotVarIndex].variable == pivotVar);
		...
		cs_assert(prevDecisionLevel <= m_topLevel);
		...
		{
			cs_assert(m_variableClauseIndices[pivotVar.raw()] == pivotVarIndex);
		...
		{
			cs_sanity(stack[pivotModTime].variable == m_nodes[pivotVarIndex].var);
			...
			cs_assert(newDecisionLevel <= prevDecisionLevel);
...
void ConflictAnalyzer::insertClause(const Literal& clause, const ConstraintGraphRelationInfo& originRelationInfo, vector<Literal>& outClauses, SolverTimestamp newTimestamp)
{
	...
	cs_sanity(clauseIndex == indexOfPredicate(outClauses.begin(), outClauses.end(), [&](auto& c) { return c.variable == clause.variable; }));

	if (clauseIndex < 0)
	{
		cs_sanity(getNodeIndexForVar(clause.variable) < 0);
		nodeIndex = m_nodes.size();

		SolverDecisionLevel newLevel = m_solver.getDecisionLevelForTimestamp(newTimestamp);
		cs_assert(newLevel <= m_topLevel);
	...
	{
		cs_assert(getNodeIndexForVar(clause.variable) == clauseIndex);
		nodeIndex = clauseIndex;

		ImplicationNode& node = m_nodes[nodeIndex];
		outClauses[clauseIndex].values.include(clause.values);
		cs_sanity(outClauses[clauseIndex].values.contains(false));
...
{
	cs_assert(m_graph != nullptr);
	if (graphNode != m_anchorGraphNode)
	{
		TopologyLink link;
		if (!m_graph->getTopologyLink(m_anchorGraphNode, graphNode, link))
		{
			return false;
		}
...
void ConflictAnalyzer::applyGraphRelation(ImplicationNode& node, const ConstraintGraphRelationInfo& originGraphInfo, const ValueSet& values, EGraphRelationType applicationType)
{
	...
	{
		cs_sanity(newTypedRel != nullptr);
		using T = decay_t<decltype(newTypedRel)>;

		auto offsetRel = createOffsetGraphRelation(originGraphInfo.sourceGraphNode, newTypedRel);
		...
		if constexpr (is_same_v<T, LiteralRelationType>)
		{
			...
			cs_verify(newTypedRel->getRelation(originGraphInfo.sourceGraphNode, relationVals));
			cs_sanity(relationVals.variable == node.var);
...
bool ConflictAnalyzer::checkRedundant(const vector<Literal>& explanation, int litIndex, uint32_t levelMask) const
{
	...
	cs_assert(m_nodes[litIndex].var == explanation[litIndex].variable);
	...
	while (!m_redundancyStack.empty())
	{
		...
		if (curNode.time >= 0)
		{
			cs_assert(stack[curNode.time].variable == curNode.var);
			cs_assert(stack[curNode.time].constraint != nullptr);
			...
			for (int i = 0; i < reasons.size(); ++i)
			{
				...
				if (!alreadySeen || !reason.values.isSubsetOf(seenValues))
				{
					...
					{
						cs_assert(stack[reasonTimestamp].variable == reason.variable);
...
SolverTimestamp ConflictAnalyzer::findLatestFalseTime(VarID var, const ValueSet& assertingValue, SolverTimestamp latestTime) const
{
	...
	while (latestTime >= 0)
	{
		cs_assert(stack[latestTime].variable == var);
...
void ConflictAnalyzer::refreshTopLevel()
{
	...
	cs_assert(m_topLevel <= prevTopLevel);
	refreshTopLevelNodeCount();
}
...
void ConflictAnalyzer::refreshTopLevelNodeCount()
{
	...
	for (auto& node : m_nodes)
	{
		cs_assert(node.level <= m_topLevel);
		if (node.level == m_topLevel)
		{
			++m_numTopLevelNodes;
		}
