...
int SegmentedVariablePropagator::insertSink(int segment, WatcherHandle handle, IVariableWatchSink* sink)
{
	...
	cs_assert(seg.end <= m_entries.size());
	cs_assert(segment == m_segments.size()-1 || m_segments[segment+1].start >= seg.end);
...
{
	cs_assert(withinSegment(segment, i));
	cs_assert(m_segments[segment].end > m_segments[segment].start);
