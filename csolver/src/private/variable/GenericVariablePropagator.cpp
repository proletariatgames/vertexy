...
WatcherHandle GenericVariablePropagator::addWatcher(IVariableWatchSink* sink, EVariableWatchType watchType)
{
	...
	cs_sanity(getTypeFromHandle(handle) == static_cast<uint32_t>(watchType));
...
bool GenericVariablePropagator::setWatcherEnabled(WatcherHandle handle, IVariableWatchSink* sink, bool enabled)
{
	...
	{
		cs_assert(watchType == HANDLE_TYPE_SINGLE_OFF);
		const uint32_t handleIndex = getIndexFromHandle(handle);
...
WatcherHandle GenericVariablePropagator::addValueWatcher(IVariableWatchSink* sink, const ValueSet& watchValues)
{
	...
	else
	{
		...
		if (segment < 0)
		{
			...
			cs_sanity(m_segmentToValueKey.size() == segment - getFirstValueSegment());
	...
	if (!freeHandles.empty())
	{
		...
		if (singleTrueBit >= 0)
		{
			cs_sanity(getTypeFromHandle(handle) == HANDLE_TYPE_SINGLE_ON);
			m_handleToSingleValue[0][getIndexFromHandle(handle)] = singleTrueBit;
		}
		else if (singleFalseBit >= 0)
		{
			cs_sanity(getTypeFromHandle(handle) == HANDLE_TYPE_SINGLE_OFF);
			m_handleToSingleValue[1][getIndexFromHandle(handle)] = singleFalseBit;
		}
		else
		{
			cs_sanity(getTypeFromHandle(handle) == HANDLE_TYPE_VALUE);
	...
	else
	{
		if (singleTrueBit >= 0)
		{
			...
			const uint32_t handleIndex = getIndexFromHandle(handle);
			cs_assert(m_handleToSingleValue[0].size() == handleIndex);
		...
		else if (singleFalseBit >= 0)
		{
			...
			const uint32_t handleIndex = getIndexFromHandle(handle);
			cs_assert(m_handleToSingleValue[1].size() == handleIndex);
		...
		else
		{
			...
			const uint32_t handleIndex = getIndexFromHandle(handle);
			cs_assert(m_handleToWatchedValuesKey.size() == handleIndex);
