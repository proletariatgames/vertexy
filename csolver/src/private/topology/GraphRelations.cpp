...
bool ClauseToLiteralGraphRelation::getRelation(int sourceNode, Literal& out) const
{
	SignedClause clause;
	if (!m_clauseRel->getRelation(sourceNode, clause))
	{
		return false;
	}
...
bool LiteralTransformGraphRelation::getRelation(int sourceNode, Literal& out) const
{
	...
	{
		if (!m_relations[i]->getRelation(sourceNode, otherVal) ||
...
bool InvertLiteralGraphRelation::getRelation(int sourceNode, Literal& out) const
{
	if (!m_inner->getRelation(sourceNode, out))
	{
		return false;
	}
	out.values.invert();
