...
#define MATCHING_SANITY_CHECK CS_SANITY_CHECKS
...
bool CardinalityConstraint::initialize(IVariableDatabase* db)
{
	...
	for (int i = 0; i < m_allVariables.size(); ++i)
	{
		...
		if (addLowerBound)
		{
			cs_sanity(!contains(m_lowerBoundVariables.begin(), m_lowerBoundVariables.end(), var));
			m_lowerBoundVariables.push_back(var);
		}

		if (addUpperBound)
		{
			cs_sanity(!contains(m_upperBoundVariables.begin(), m_upperBoundVariables.end(), var));
...
void CardinalityConstraint::backtrack(const IVariableDatabase* db, SolverDecisionLevel level)
{
	...
	cs_assert(c == m_numUpperBoundVarsOutsideUBC);
}
...
CardinalityConstraint::BacktrackInfo& CardinalityConstraint::backtrackRecord(SolverDecisionLevel level)
{
	...
	{
		cs_assert(m_backtrackStack.back().level < level);
...
bool CardinalityConstraint::onVariableNarrowed(IVariableDatabase* db, VarID variable, const ValueSet& prevValues, bool&)
{
	...
	{
		cs_sanity(db->getDecisionLevel() >= m_fullySatisfiedLevel);
		return true;
	}
	...
	if (m_hasUpperBoundConstraint)
	{
		...
		else if (prevValues.anyPossible(m_upperBoundConstrainedValues))
		{
			...
			cs_sanity(contains(m_upperBoundVariables.begin(), m_upperBoundVariables.end(), variable));
			m_numUpperBoundVarsOutsideUBC++;
			cs_assert(m_numUpperBoundVarsOutsideUBC <= m_upperBoundVariables.size());
		}
	}
...
{
	cs_assert(m_numUpperBoundVarsOutsideUBC+m_numUnitSCCs <= m_upperBoundVariables.size());
...
bool CardinalityConstraint::getMaximalMatching(IVariableDatabase* db)
{
	for (int varIndex : m_upperBoundProcessList)
	{
		...
		cs_assert(!db->isInContradiction(m_upperBoundVariables[varIndex]));
	...
	for (int i = 0; i < m_upperBoundVariables.size(); ++i)
	{
		...
		{
			cs_assert(m_matchingGraph.hasBipartiteEdge(i, (*it)-m_minDomainValue));
		}
	}
	#endif
...
{
	cs_assert(m_upperBoundProcessList.size() > 0);

	#if MATCHING_SANITY_CHECK
	for (int i = 0; i < m_sccToNode.size(); ++i)
	{
		cs_assert(indexOf(m_sccToNode.rbegin(), m_sccToNode.rend(), m_sccToNode[i]) == m_sccToNode.size()-i-1);
	}
	#endif
	...
	for (int varIndex = 0; varIndex < m_upperBoundVariables.size(); ++varIndex)
	{
		...
		cs_assert(m_valueToSumInMatching[matchingValue] <= m_trimmedMaxs[matchingValue]);
	}
		...
		for (int value = m_minDomainValue; value <= m_maxDomainValue; ++value)
		{
			...
			if (maxCapacity < m_upperBoundVariables.size())
			{
				if (m_valueToSumInMatching[relValue] < maxCapacity)
				{
					...
					{
						cs_assert(valueNodeToValue(dest) == value);
						visitor(dest);
					}
				}
			}
			else
			{
				...
				cs_assert(m_valueToSumInMatching[relValue] < maxCapacity);
...
vector<Literal> CardinalityConstraint::explainLowerBoundPropagation(const NarrowingExplanationParams& params) const
{
	...
	cs_assert(!constrainedVals.isZero());
...
bool CardinalityConstraint::explainConflict(const IVariableDatabase* db, vector<Literal>& outClauses) const
{
	...
	{
		cs_assert(m_failedLowerBoundMatching);
