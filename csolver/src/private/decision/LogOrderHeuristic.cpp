...
bool LogOrderHeuristic::getNextDecision(SolverDecisionLevel inLevel, VarID& var, ValueSet& chosenValues)
{
	...
	{
		CS_LOG("Waiting to explore unexplored dead-end starting at %d (D=%d)", m_log->getDecision(m_nextDecision).level, m_nextDecision);
		return false;
	}
	...
	{
		++numToSkip;
		cs_assert_msg(m_nextDecision+numToSkip < m_log->getNumDecisions(), "Skipped over a solution found in log");
	}
	if (numToSkip > 0)
	{
		CS_LOG("Solver backtracked: Skipping over dead-end branch to Decision %d (%d skipped) level %d", m_nextDecision + numToSkip, numToSkip, m_log->getDecision(m_nextDecision + numToSkip).level);
	...
	while (!m_solver.getVariableDB()->getPotentialValues(m_log->getDecision(m_nextDecision).variable)[m_log->getDecision(m_nextDecision).valueIndex])
	{
		...
		cs_assert_msg(i < m_log->getNumDecisions(), "Skipped solution found!");

		CS_LOG("Solver prevented assignment: Skipping over dead-end branch to Decision %d (%d skipped) level %d", i, i - m_nextDecision, m_log->getDecision(i).level);
		m_nextDecision = i;
	...
	while (true)
	{
		...
		CS_LOG("Skipping already assigned: %s=%d", m_solver.getVariableName(m_log->getDecision(m_nextDecision).variable).c_str(), solvedValue);
		cs_assert(solvedValue == m_log->getDecision(m_nextDecision).valueIndex);
		++m_nextDecision;
		++m_levelOffset;
		++curLevel;
		if (m_nextDecision >= m_log->getNumDecisions())
	...
	cs_sanity(m_log->getDecision(m_nextDecision).level == curLevel);
