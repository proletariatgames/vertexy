...
static constexpr bool EXPLANATION_SANITY_CHECK = CS_SANITY_CHECKS;
// Whether graph-learning is enabled. When enabled, some learned constraints can be promoted to all nodes on a graph.
...
{
	cs_assert(m_currentStatus == EConstraintSolverResult::Uninitialized);
	...
	cs_assert(m_variableDomains.size() == varId.raw());
	m_variableDomains.push_back(domain);

	cs_assert(m_variableToDecisionLevel.size() == varId.raw());
	m_variableToDecisionLevel.push_back(0);

	// Allocate the best type of propagator based on domain size of the variable
	cs_assert(m_variablePropagators.size() == varId.raw());
	...
	cs_assert(m_variableToGraphs.size() == varId.raw());
	m_variableToGraphs.push_back({});
...
{
	cs_assert(!potentialValues.empty()); // can't start with an empty domain
...
{
	cs_assert(getCurrentStatus() == EConstraintSolverResult::Solved);
...
EConstraintSolverResult ConstraintSolver::startSolving()
{
	...
	else if (m_currentStatus == EConstraintSolverResult::Solved)
	{
		...
		{
			VarID varID(i);
			cs_sanity(m_variableDB.getPotentialValues(varID).isSingleton());
	...
	{
		cs_assert_msg(false, "startSolving called in bad state!");
		m_currentStatus = EConstraintSolverResult::Unsatisfiable;
	}
...
EConstraintSolverResult ConstraintSolver::step()
{
	...
	if (!propagate())
	{
		...
		cs_assert_msg(lastNarrowedConstraint, "Detected sink instead of constraint as conflict source");
		...
		cs_assert(backtrackLevel < getCurrentDecisionLevel());
		...
		//CS_LOG("Learned constraint %d: %s", learnedConstraint->getID(), clauseConstraintToString(*learnedConstraint).c_str());
		cs_assert(learnedConstraint->getNumLiterals() > 0);
		if (learnedConstraint->getNumLiterals() == 1)
		{
			// No need to keep this around, just propagate it and forget it
			cs_assert(backtrackLevel == 0);
			...
			cs_assert(!learnedConstraint->isLocked());

			cs_assert(m_constraints[learnedConstraint->getID()].get() == learnedConstraint);
	...
	else
	{
		...
		else
		{
			...
			if (CS_LOG_ACTIVE())
			{
				...
				{
					CS_LOG("Level %d Step %d Var:%s[%d] Value:%s", getCurrentDecisionLevel(), m_stats.stepCount, m_variableDB.getVariableName(pickedVar).c_str(), pickedVar.raw(), valueSetToString(pickedVar, pickedValue).c_str());
				}
			}
			...
			if (m_outputLog != nullptr)
			{
				...
				cs_sanity(valueIndex >= 0);
				cs_sanity(pickedValue.lastIndexOf(true) == valueIndex);
				m_outputLog->addDecision(getCurrentDecisionLevel(), pickedVar, valueIndex);
			}

			cs_assert(m_variableToDecisionLevel[pickedVar.raw()] == 0);
			...
			cs_sanity(!m_variableDB.getPotentialValues(pickedVar).isSubsetOf(pickedValue));

			bool success = m_variableDB.constrainToValues(pickedVar, pickedValue, nullptr);
			cs_assert(success); // If this goes off, the strategy did not return a potential value
		}
	}
...
bool ConstraintSolver::emptyVariableQueue()
{
	...
	while (!m_variablePropagationQueue.empty())
	{
		...
		cs_assert(m_variableQueuedSet[item.variable.raw()]);
		m_variableQueuedSet[item.variable.raw()] = false;

		cs_assert(stack[item.timestamp].variable == item.variable);
...
bool ConstraintSolver::emptyConstraintQueue()
{
	while (!m_constraintPropagationQueue.empty())
	{
		...
		cs_assert(m_constraintQueuedSet[constraintID]);
...
{
	cs_assert(decisionLevel < getCurrentDecisionLevel());
	// CS_LOG("Level %d Step %d BACKTRACK to %d", GetCurrentDecisionLevel(), Stats.StepCount, DecisionLevel);
	...
	while (getCurrentDecisionLevel() > decisionLevel)
	{
		...
		{
			cs_assert(m_variableToDecisionLevel[decisionVar.raw()] != 0);
...
void ConstraintSolver::sanityCheckExplanation(SolverTimestamp modificationTime, const vector<Literal>& explanation) const
{
	if (EXPLANATION_SANITY_CHECK)
	{
		...
		cs_assert(pivotIndex >= 0);
		{
			...
			cs_assert(!explanation[pivotIndex].values.anyPossible(removedBits));
		}

		for (int i = 0; i < explanation.size(); ++i)
		{
			cs_assert(explanation[i].values.contains(false));
			if (i != pivotIndex)
			{
				auto& varsForConstraint = getVariablesForConstraint(mod.constraint);
				cs_assert(contains(varsForConstraint.begin(), varsForConstraint.end(), explanation[i].variable));
				const ValueSet& argValueBefore = m_variableDB.getValueBefore(explanation[i].variable, modificationTime);
				cs_assert(!argValueBefore.anyPossible(explanation[i].values));
			}
		}
	}
}
...
ClauseConstraint* ConstraintSolver::learn(const vector<Literal>& explanation, const ConstraintGraphRelationInfo* relationInfo)
{
	...
	if (learnedCons.getNumLiterals() > 1)
	{
		...
		{
			CS_WARN("Duplicate clause %d created for %d", learnedCons.getID(), (*foundExisting)->getID());
		}
		...
		if (learnedCons.getLBD() <= MAX_PERMANENT_CONSTRAINT_LBD || canPromoteToGraph)
		{
			...
			{
				cs_sanity(m_constraintsToPromoteToGraph.find(&learnedCons) == m_constraintsToPromoteToGraph.end());
				m_constraintsToPromoteToGraph[&learnedCons] = 0;
			}
		}
		else
		{
			cs_assert(!learnedCons.isPermanent());
...
void ConstraintSolver::disableWatcherUntilBacktrack(WatcherHandle handle, VarID variable, IVariableWatchSink* sink)
{
	...
	if (m_variablePropagators[variable.raw()]->setWatcherEnabled(handle, sink, false))
	{
		if (curLevel > 0)
		{
			cs_assert(m_disabledWatchMarkers.empty() || m_disabledWatchMarkers.back().level <= curLevel);
...
{
	cs_assert(varID.isValid());
	cs_assert(watchValues.size() == m_variableDB.getDomainSize(varID));
...
{
	cs_assert(constraint.isLearned());
...
bool ConstraintSolver::promoteConstraintToGraph(ClauseConstraint& constraint, int& startNode)
{
	cs_assert(constraint.isLearned());
	cs_assert(constraint.isPermanent());
	cs_assert(constraint.getGraph() != nullptr);
	cs_assert(constraint.isPromotableToGraph());

	++m_stats.numConstraintPromotions;

	auto& graph = constraint.getGraph();

	int promotingNode = constraint.getGraphRelationInfo()->sourceGraphNode;
	...
	cs_sanity(startNode < graph->getNumNodes());
	for (int nodeIndex = startNode; nodeIndex < graph->getNumNodes(); ++nodeIndex)
	{
	...
	if (CS_LOG_ACTIVE() && LOG_GRAPH_PROMOTIONS)
	{
		ConstraintGraphRelationInfo tempInfo;
		vector<Literal> tempLits;
		cs_verify(createLiteralsForGraphPromotion(constraint, constraint.getGraphRelationInfo()->sourceGraphNode, tempInfo, tempLits));
		...
		for (auto& entry : constraint.getGraphRelationInfo()->relations)
		{
			visit([&](auto&& typedRel)
			{
				...
				{
					VarID varID;
					cs_verify(typedRel->getRelation(constraint.getGraphRelationInfo()->sourceGraphNode, varID));
		...
		CS_LOG("Promoted constraint %d:\n%s%d Created, %d dupes\n", constraint.getID(), relationStr.c_str(), numCreated, numDuplicates);
		if (numCreated == 0)
		{
			CS_LOG("Could not promote %s", clauseConstraintToString(constraint).c_str());
		}
	}
	...
	if (success)
	{
		...
		startNode = graph->getNumNodes();
	}
	return success;
}

bool ConstraintSolver::createLiteralsForGraphPromotion(const ClauseConstraint& promotingCons, int destNode, ConstraintGraphRelationInfo& outRelInfo, vector<Literal>& outLits) const
{
	cs_assert(promotingCons.getGraph() != nullptr);
	outLits.clear();
	outRelInfo.reset(promotingCons.getGraph(), destNode);
	...
	for (auto& entry : promotingCons.getGraphRelationInfo()->relations)
	{
		visit([&](auto&& typedRel)
		{
			...
			if constexpr (is_same_v<T, shared_ptr<const IGraphRelation<VarID>>>)
			{
				...
				cs_verify(typedRel->getRelation(promotingCons.getGraphRelationInfo()->sourceGraphNode, correspondingVar));
				...
				cs_sanity(correspondingLit != nullptr);

				VarID var;
				if (!typedRel->getRelation(destNode, var))
				{
					success = false;
					return;
				}
			...
			else if constexpr (is_same_v<T, shared_ptr<const IGraphRelation<Literal>>>)
			{
				...
				cs_verify(typedRel->getRelation(promotingCons.getGraphRelationInfo()->sourceGraphNode, correspondingClause));
				const ValueSet& correspondingVarInitialVals = m_variableDB.getInitialValues(correspondingClause.variable);

				Literal clause;
				if (!typedRel->getRelation(destNode, clause))
				{
					success = false;
					return;
				}
				...
				{
					cs_assert(clause.values.contains(true));
...
void ConstraintSolver::purgeConstraints()
{
	...
	{
		cs_assert(lhs->getNumLiterals() >= 2);
		cs_assert(rhs->getNumLiterals() >= 2);
	...
	for (i = m_temporaryLearnedConstraints.size() - 1; i >= 0 && m_temporaryLearnedConstraints.size() > numRemaining; --i)
	{
		...
		if (!cons->isLocked())
		{
			...
			cs_assert(m_constraints[cons->getID()].get() == cons);
...
void ConstraintSolver::sanityCheckValid()
{
	for (int i = 0; i < m_constraints.size(); ++i)
	{
		...
		{
			cs_assert_msg(false, "Constraint %d conflicting after solution found!", constraint->getID());
		}
	}
}
	...
	if (TEST_GRAPH_PROMOTIONS)
	{
		...
		for (ClauseConstraint* constraint : allLearnedConstraints)
		{
			if (constraint->isPromotableToGraph() && !constraint->isPromotedFromGraph())
			{
				...
				cs_assert_msg(promoted, "Invalid graph constraint %d: %s", constraint->getID(), clauseConstraintToString(*constraint).c_str());
			}
		}
	}
}
...
{
	CS_LOG("%s", m_stats.toString(verbose).c_str());
}
...
void ConstraintSolver::debugSaveSolution(const wchar_t* filename) const
{
	...
	CS_LOG("Wrote solution to %s", filename);
	file.close();
}

void ConstraintSolver::debugAttemptSolution(const wchar_t* filename)
{
	CS_WARN("Attempting predefined solution %s...", filename);
	...
	{
		int var, value;
		cs_verify(2 == swscanf_s(line.c_str(), TEXT("%d %d"), &var, &value));
	...
	CS_WARN("Finished predefined solution!", filename);
}
