...
public:
	...
	virtual bool getRelation(int sourceNode, T& out) const = 0;
...
public:
	...
	virtual bool getRelation(int node, T& out) const override
	{
		int mapped;
		if (!m_relationFirst->getRelation(node, mapped))
		{
			return false;
		}
...
public:
	...
	virtual bool getRelation(int sourceNode, T& out) const override
	{
		if (!m_data->getSource()->isValidNode(sourceNode))
		{
			return false;
		}
		out = m_data->get(sourceNode);
		return true;
	}
...
public:
	...
	{
		vector<T> found;
		if (!arrayRel->getRelation(sourceNode, found))
		{
			return false;
		}
...
public:
	...
	virtual bool getRelation(int sourceNode, T& out) const override
	{
		...
		T val;
		if (!m_relations[0]->getRelation(sourceNode, val))
		{
			return false;
		}
		...
		{
			T otherVal;
			if (!m_relations[i]->getRelation(sourceNode, otherVal) || val != otherVal)
			{
				return false;
			}
		}
...
public:
	...
	virtual bool getRelation(int sourceNode, Literal& out) const override;
	virtual wstring toString() const override;
...
public:
	...
	virtual bool getRelation(int sourceNode, int& out) const override
	{
		int destNode;
		if (!m_link.resolve(m_topo, sourceNode, destNode))
		{
			return false;
		}
...
public:
	...
	{
		int destNode;
		if (!m_link.resolve(m_data->getSource(), sourceNode, destNode))
		{
			return false;
		}

		out = m_data->get(destNode);
		return true;
	}
...
class TGraphEdgeToNodeIndexRelation : public IGraphRelation<int>
{
public:
	explicit TGraphEdgeToNodeIndexRelation(const shared_ptr<EdgeTopology>& edgeTopology)
	...
	virtual bool getRelation(int edgeNode, int& out) const override
	{
		int nodeFrom, nodeTo;
		bool bidirectional;
		m_edgeTopology->getSourceEdgeForNode(edgeNode, nodeFrom, nodeTo, bidirectional);
	...
	virtual bool equals(const shared_ptr<const ITopology>& topology, const IGraphRelation<int>& rhs) const override
	{
		...
		auto typedRHS = dynamic_cast<const TGraphEdgeToNodeIndexRelation<INCOMING_EDGE>*>(&rhs);
		return typedRHS != nullptr && typedRHS->m_edgeTopology == m_edgeTopology;
	}

	virtual wstring toString() const override
	{
		return INCOMING_EDGE ? TEXT("IncomingNodeForEdge") : TEXT("OutgoingNodeForEdge");
	}
...
{
public:
	TVertexEdgeToEdgeGraphVertexGraphRelation(const shared_ptr<TOPO_TYPE>& nodeTopo, const shared_ptr<EdgeTopology>& edgeTopo, int edgeIndex)
		: m_nodeTopo(nodeTopo)
		, m_edgeTopo(edgeTopo)
	...
	virtual bool getRelation(int node, int& out) const override
	{
		int destNode;
		if constexpr (!INCOMING_EDGE)
		{
			if (!m_nodeTopo->getOutgoingDestination(node, m_edgeIndex, destNode))
			{
				return false;
			}
		}
		...
		{
			if (!m_nodeTopo->getIncomingSource(node, m_edgeIndex, destNode))
			{
				return false;
			}
		}

		int edgeNode = m_edgeTopo->getNodeForSourceEdge(node, destNode);
		cs_assert(m_edgeTopo->isValidNode(edgeNode));
	...
	virtual bool equals(const shared_ptr<const ITopology>& topology, const IGraphRelation<int>& rhs) const override
	{
		...
		return typedRHS != nullptr && typedRHS->m_nodeTopo == m_nodeTopo && typedRHS->m_edgeTopo == m_edgeTopo && typedRHS->m_edgeIndex == m_edgeIndex;
	}

	virtual wstring toString() const override
	{
		return INCOMING_EDGE
				   ? wstring{wstring::CtorSprintf(), TEXT("IncomingEdge(%s)"), m_nodeTopo->edgeIndexToString(m_edgeIndex).c_str()}
		: wstring{wstring::CtorSprintf(), TEXT("OutgoingEdge(%s)"), m_nodeTopo->edgeIndexToString(m_edgeIndex).c_str()};
	}

protected:
	shared_ptr<TOPO_TYPE> m_nodeTopo;
