...
public:
	...
	{
		cs_assert_msg((index >= 0) & (index < m_size), "TSparseSet index out of bounds: %i from a set of size %i", index, m_size); // & for one branch
	}
	...
	{
		cs_sanity(m_size <= m_dense.size());
		if (bInsertion)
		{
			cs_sanity(stamp <= m_size);
		}
		else
		{
			cs_sanity(stamp >= m_size);
	...
	{
		cs_sanity(m_map.isValidIndex(value));
		int index = m_map[value];
