...
public:
	...
	TValueBitset& operator=(TValueBitset&& other) noexcept
	{
		...
		{
			cs_sanity(m_numBits <= MAX_INLINE_BITS);
	...
	void pad(int32_t numBits, bool fillValue)
	{
		...
		cs_sanity(at(numBits-1) == fillValue);
	}

	void setRange(int startBit, int endBit, bool fillValue)
	{
		cs_sanity(endBit >= startBit);
		cs_assert(startBit < m_numBits);
		cs_assert(endBit <= m_numBits);
		...
		cs_sanity(lastWord < numWordsRequired(m_numBits));
	...
	{
		cs_assert(index >= 0 && index < m_numBits);
	...
	{
		cs_assert(other.size() >= size());
		cs_sanity(lastBit > firstBit);
		cs_sanity(lastBit < size());
...
private:
	inline void setSize(int bitCount)
	{
		if (bitCount > MAX_INLINE_BITS && bitCount > m_numBits)
		{
			...
			{
				cs_sanity(newNumWords > numWordsRequired(m_numBits));
				if (m_dataPtr != m_inlineWords)
				{
					delete [] m_dataPtr;
				}
...
struct hash<csolver::TValueBitset<ALLOCATOR, NUM_INLINE_WORDS, WORD_TYPE>>
{
	inline size_t operator()(const csolver::TValueBitset<ALLOCATOR, NUM_INLINE_WORDS, WORD_TYPE>& val) const
	{
		size_t hash = 0;
