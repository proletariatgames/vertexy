...
public:
	...
	void fillVariableGraph(const shared_ptr<TTopologyVertexData<VarID>>& data, const SolverVariableDomain& variableDomain, const wstring& namePrefix)
	{
		...
		{
			wstring varName = namePrefix + graph->nodeIndexToString(i);
	...
	//
	// Create a constraint across an entire graph. All nodes in the graph will share the same constraints.
	//
	// To refer to nodes at relative locations within the graph, use FTopologyLink instead of FVarID, or FSignedGraphClause instead of FSignedClause.
	// You can also refer to a variable outside the graph, by passing FVarID/FSignedClause as normal.
	//
	// Example 1: Making an inequality where all nodes in graph should be greater than the node "below" them:
	...
	//		);
	//
	// Example 2: Making an inequality where all nodes in the graph should be greater than another variable's value:
...
protected:
	...
	bool promoteConstraintToGraph(ClauseConstraint& constraint, int& startNode);
	bool createLiteralsForGraphPromotion(const ClauseConstraint& promotingCons, int destNode, ConstraintGraphRelationInfo& outRelInfo, vector<Literal>& outLits) const;
	...
	inline bool isVariableInGraph(VarID varID, int graphID) const
	{
		cs_assert(varID.isValid());
		cs_assert(graphID >= 0);
		cs_assert(graphID < m_graphs.size());
	...
	// Queue of constraints ready to be propagated across graphs, mapped to the next node index to be processed.
	...
	// of a variable in a topology, and create constraints for each node in the topology by resolving the
	// the relation into a concrete FVarID per node.
	//
	...
	{
		cs_assert(arg.relation == nullptr);
	}
	...
	{
		auto translatedArg = GraphArgumentTransformer::transformGraphArgument(relationInfo.sourceGraphNode, arg);
	...
	auto translateGraphConsArgument(const GraphConstraintID& id, ConstraintGraphRelationInfo& relationInfo, bool& success)
	{
		...
		cons = graphCons->get(relationInfo.sourceGraphNode);
	...
	template <typename T, typename... ArgsType>
	ISolverConstraint* maybeInstanceGraphConstraint(const shared_ptr<ITopology>& graph, uint32_t nodeIndex, ArgsType&&... args)
	{
		ConstraintGraphRelationInfo graphRelationInfo(graph, nodeIndex);
