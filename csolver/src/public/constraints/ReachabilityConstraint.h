...
public:
	...
	struct ReachabilityFactory
	{
		static ReachabilityConstraint* construct(
			...
			// Graph of nodes where reachability is calculated
			const shared_ptr<TTopologyVertexData<VarID>>& sourceGraphData,
			// Values of nodes in SourceGraph that establish it as a reachability source
			const vector<int>& sourceValues,
			// Values of nodes in SourceGraph that establish it as needing to be reachable from a source
			const vector<int>& needReachableValues,
			// The variables for each edge of source graph
			const shared_ptr<TTopologyVertexData<VarID>>& edgeGraphData,
			// Values of nodes in the edge graph establishing that edge as "off"
...
protected:
	...
	EReachabilityDetermination determineReachability(const IVariableDatabase* db, int node);
	bool processNodeVariableChange(IVariableDatabase* db, VarID variable);
	void updateGraphsForEdgeChange(IVariableDatabase* db, VarID variable);
	void onReachabilityChanged(int nodeIndex, VarID sourceVar, bool inMinGraph);
	void sanityCheckUnreachable(IVariableDatabase* db, int nodeIndex);
	...
	hash_map<VarID, WatcherHandle> m_nodeWatchHandles;
	...
	vector<VarID> m_nodeProcessList;
	...
	hash_map<VarID, int> m_variableToSourceNodeIndex;
