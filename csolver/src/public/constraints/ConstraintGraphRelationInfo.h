...
public:
	...
	ConstraintGraphRelationInfo(const shared_ptr<ITopology>& graph, int sourceNode);

	void clear();
	void reset(const shared_ptr<ITopology>& graph, int sourceNode);
	...
	// The node within the graph this constraint was instantiated for.
	int sourceGraphNode;
