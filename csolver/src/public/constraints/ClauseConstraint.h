...
#define CLAUSE_DEBUG_INFO CS_SANITY_CHECKS
...
public:
	...
	inline void lock()
	{
		cs_assert(m_extendedInfo.get());
		cs_assert(m_extendedInfo->isLearned);
		cs_assert(m_extendedInfo->lockCount < 0xFFFF);
	...
	inline void unlock()
	{
		cs_assert(m_extendedInfo.get());
		cs_assert(m_extendedInfo->isLearned);
		cs_assert(m_extendedInfo->lockCount > 0);
		--m_extendedInfo->lockCount;
	}
	...
	{
		cs_assert(isLearned());
		cs_assert(m_graphRelationInfo != nullptr);
		cs_assert(m_extendedInfo != nullptr);
