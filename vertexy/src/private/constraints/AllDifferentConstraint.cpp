...
bool AllDifferentConstraint::initialize(IVariableDatabase* db)
{
	...
	if (!m_useWeakPropagation)
	{
		m_hallIntervalPropagator = make_unique<HallIntervalPropagation>(m_maxDomainSize);
...
bool AllDifferentConstraint::checkConflicting(IVariableDatabase* db) const
{
	if (!m_useWeakPropagation)
	{
		...
		if (!m_hallIntervalPropagator->checkAndPrune(tempBounds, [&](int, int) { return true; }))
		{
			return false;
		}

		if (!m_hallIntervalPropagator->checkAndPrune(tempInvBounds, [&](int, int) { return true; }))
		{
			return false;
		}
	}
...
{
	vxy_assert(db->isSolved(solvedVar));
...
bool AllDifferentConstraint::checkBoundsConsistency(IVariableDatabase* db, const vector<VarID>& unsolvedVariables)
{
	...
	if (!m_hallIntervalPropagator->checkAndPrune(m_bounds, excludeLessThan))
	{
		return false;
	}

	if (!m_hallIntervalPropagator->checkAndPrune(m_invBounds, excludeGreaterThan))
	{
		return false;
	}
