...
void IConstraint::explain(const NarrowingExplanationParams& params, vector<Literal>& outExplanation) const
{
    ...
    outExplanation.reserve(constraintVars.size());

    bool foundPropagated = false;
    for (int i = 0; i < constraintVars.size(); ++i)
    {
        VarID arg = constraintVars[i];
        outExplanation.push_back(Literal(arg, params.database->getPotentialValues(arg)));
        outExplanation.back().values.invert();

        if (arg == params.propagatedVariable)
        {
            foundPropagated = true;
            outExplanation.back().values.pad(params.propagatedValues.size(), false);
            outExplanation.back().values.include(params.propagatedValues);
        }
    }
